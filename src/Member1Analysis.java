 ...
 *   3. Compute betweenness centrality (Brandes, approximated)
 *   4. Compute closeness centrality (BFS, sampled)
 *   5. Identify "hidden gems" — low citation count but high betweenness
 *   6. Write results to outputs/tables/
 */
public class Member1Analysis {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.printf("WCC: %,d nodes, %,d edges (%.1f%% of original)%n",
                ...
                100.0 * G.nodeCount() / wccNodes.size());

        // ── 2. Basic stats ─────────────────────────────────────────────────
        NetworkStats.printSummary(G);

        // Raw citation counts (in-degree)
        Map<String, Integer> citationCount = new HashMap<>();
        for (String node : G.nodes()) citationCount.put(node, G.inDegree(node));
        ...
        // ── 4. Closeness centrality (3000 sampled nodes) ───────────────────
        ...
        List<String> hiddenGems = new ArrayList<>();
        for (String node : G.nodes()) {
            if (citationCount.get(node) <= medianCite && betweenness.get(node) >= bwTop10) {
                hiddenGems.add(node);
            }
        }
        hiddenGems.sort((a, b) -> Double.compare(betweenness.get(b), betweenness.get(a)));

        System.out.printf("Median citations: %d   Top-10%% betweenness threshold: %.6f%n",
                medianCite, bwTop10);
        System.out.printf("Hidden gems found: %,d%n", hiddenGems.size());
        System.out.println("\nTop 15 hidden gems (low citations, high betweenness):");
        System.out.printf("%-12s  %8s  %12s  %6s%n", "PaperID", "Citations", "Betweenness", "Year");
        System.out.println("-".repeat(50));
        for (int i = 0; i < Math.min(15, hiddenGems.size()); i++) {
            String node = hiddenGems.get(i);
            Integer yr = G.getYear(node);
            System.out.printf("%-12s  %8d  %12.6f  %6s%n",
                    node, citationCount.get(node), betweenness.get(node),
                    yr != null ? yr.toString() : "?");
        }

        // ── 6. Spearman correlation ────────────────────────────────────────
        double rhoB = NetworkStats.spearmanCorrelation(citationCount, betweenness);
        System.out.printf("%nSpearman(citations, betweenness) = %.4f%n", rhoB);

        Map<String, Double> closenessAll = new HashMap<>(closeness);
        double rhoC = NetworkStats.spearmanCorrelation(citationCount, closenessAll);
        System.out.printf("Spearman(citations, closeness)   = %.4f  (sampled nodes only)%n", rhoC);

        // ── 7. Write CSVs ──────────────────────────────────────────────────
        Files.createDirectories(Path.of(OUT_DIR));

        writeCSV(G, citationCount, betweenness, closeness, OUT_DIR + "/member1_metrics.csv");
        writeHiddenGems(G, hiddenGems, citationCount, betweenness, OUT_DIR + "/hidden_gems_betweenness.csv");
    ...
    private static void printTop10(CitationGraph G, Map<String, Double> scores,
                                   Map<String, Integer> citations, String label) {
        ...
        for (Map.Entry<String, Double> e : NetworkStats.topK(scores, 10)) {
            ...
            System.out.printf("%-12s  %8d  %12.6f  %6s%n",
                    e.getKey(), citations.getOrDefault(e.getKey(), 0),
                    e.getValue(), yr != null ? yr.toString() : "?");
        }
    }
    ...
    private static void writeCSV(CitationGraph G,
        ...
        try (PrintWriter pw = new PrintWriter(Files.newBufferedWriter(Path.of(path)))) {
            pw.println("paper_id,citation_count,betweenness,closeness,year");
            for (String node : G.nodes()) {
                Integer yr = G.getYear(node);
                Double cl = closeness.get(node);
                pw.printf("%s,%d,%.8f,%s,%s%n",
                        node,
                        citations.get(node),
    ...
    private static void writeHiddenGems(CitationGraph G, List<String> gems,
                                         Map<String, Integer> citations,
                                         Map<String, Double> betweenness,
                                         String path) throws IOException {
        try (PrintWriter pw = new PrintWriter(Files.newBufferedWriter(Path.of(path)))) {
